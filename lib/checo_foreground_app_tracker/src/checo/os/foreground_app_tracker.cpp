...
ForegroundAppTracker::ForegroundAppTracker(QObject *parent) : QObject(parent)
{
#ifdef WIN32
...
ForegroundAppTracker::~ForegroundAppTracker()
{
    ...
    const auto delHook = std::find_if(winHooks.begin(), winHooks.end(),
        [this](const std::pair<HWINEVENTHOOK, ForegroundAppTracker *> checkHook)
        { return checkHook.second == this; });
