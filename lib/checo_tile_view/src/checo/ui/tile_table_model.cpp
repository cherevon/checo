...
TileTableModel::TileTableModel(const std::vector<Tile> &tiles, QObject *parent) : TileTableModel(parent)
{
    setTiles(tiles);
    refreshRunningTiles();
}
...
void TileTableModel::refreshRunningTiles()
{
    ...
    std::list<QFileInfo> runningProcesses = checo::os::ApplicationInfo::getRunningWindows();
